...
def test_transform_data(sample_patient_data, sample_condition_data, sample_encounter_data):
    ...
    assert df_condition_transformed.iloc[0]["diagnosis_status"] == "confirmed"
    assert df_condition_transformed.iloc[0]["condition_code"] == "C001"
    assert df_condition_transformed.iloc[0]["condition_description"] == "Hypertension"
    assert df_condition_transformed.iloc[0]["patient_id"] == "123"
    assert df_condition_transformed.iloc[0]["encounter_id"] == "ENC001"
    ...
    assert df_encounter_transformed.iloc[0]["care_setting"] == "outpatient"
    assert df_encounter_transformed.iloc[0]["visit_type_code"] == "AMB"
    assert df_encounter_transformed.iloc[0]["visit_type_description"] == "Ambulatory"
    assert df_encounter_transformed.iloc[0]["patient_id"] == "123"
    assert df_encounter_transformed.iloc[0]["facility_name"] == "General Hospital"
    assert df_encounter_transformed.iloc[0]["provider_name"] == "Health System"
    assert df_encounter_transformed.iloc[0]["visit_start_time"] == "2023-01-01T10:00:00"
    assert df_encounter_transformed.iloc[0]["visit_end_time"] == "2023-01-01T11:00:00"
